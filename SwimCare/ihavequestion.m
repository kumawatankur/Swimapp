...
#import "ihavequestion.h"
#import "Dashboard.h"

@interface ihavequestion ()
{
    NSArray*c_name;
}
...
- (IBAction)submit_btn:(id)sender
{
    ...
    if (!error)
    {
        ...
        //Here YOUR URL
        NSMutableURLRequest *request = [[NSMutableURLRequest alloc] initWithURL:[NSURL URLWithString:@"http://shantinews.com/toshow/swimcare/ws/i_have_question.php"]];
        ...
        //Pass The String to server(YOU SHOULD GIVE YOUR PARAMETERS INSTEAD OF MY PARAMETERS)
        NSString *userUpdate =[NSString stringWithFormat:@"db_user=%@&c_name=%@&detail=%@&",[[NSUserDefaults standardUserDefaults]stringForKey:@"Loginuser"],_tx_name.text,_comment.text, nil];
